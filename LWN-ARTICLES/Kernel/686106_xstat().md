---
title: xstat()
url: https://lwn.net/Articles/686106/
date: "May 4, 2016"
...
The proposed [`xstat()` system call](<https://lwn.net/Articles/394298/>), which is meant to extend the functionality of the [`stat()`](<http://man7.org/linux/man-pages/man2/stat.2.html>) call to get additional file-status information, has been discussed quite a bit over the years, but has never been merged. The main impediment seems to be a lot of bikeshedding about how much information—and which specific pieces—will be returned. David Howells led a short filesystem-only discussion on `xstat()` at the 2016 Linux Storage, Filesystem, and Memory-Management Summit. 
...
[ ![\[David Howells\]](https://static.lwn.net/images/2016/lsf-howells-sm.jpg) ](<https://lwn.net/Articles/686126/>)
...
As it turns out, Howells [posted a new patch set](<https://lwn.net/Articles/685519/>) after LSFMM that reintroduces `xstat()` as the [`statx()` system call](<https://lwn.net/Articles/685791/#statx>).
