---
title: "2.6.39 merge window, part 2"
url: https://lwn.net/Articles/434637/
date: "March 23, 2011"
category: "Releases-2.6.39"
...
As of this writing, some 5,500 non-merge changesets have been merged into the mainline since [last week's 2.6.39 merge window summary](<https://lwn.net/Articles/433854/>). A wide-ranging set of new features, cleanups, and performance improvements has been added to the kernel. Some of the more significant user-visible changes include: 
  ...
  * The size of the initial congestion window in the TCP stack has been increased, a change which should lead to shorter latencies for the loading of web pages and other server-oriented tasks. See [this article](<https://lwn.net/Articles/427104/>) for details. 
...
It behaves like `sync()` with the exception that only the filesystem containing `fd` will be flushed to persistent storage. 
  ...
  * The [transcendent memory](<https://lwn.net/Articles/340080/>) core has been added to the staging tree. Along with it came "zcache," a compressed in-memory caching mechanism. 
  ...
  * The [CHOKe flow scheduler](<https://lwn.net/Articles/422477/>) and the Stochastic Fair Blue scheduler have been added to the networking code. 
...
Changes visible to kernel developers include: 
  ...
  * The [dynamic debug mechanism](<https://lwn.net/Articles/434833/>) has some new control flags allowing for control over whether the function name, line number, module name, and current thread ID are printed. 
  ...
  * The ["pstore" filesystem](<https://lwn.net/Articles/434821/>) provides access to platform-specific persistent storage which can be used to carry information across reboots. 
  ...
  * There is a whole new set of functions which are the preferred way to convert strings to integer values; see [this article](<https://lwn.net/Articles/435022/>) for details. 

  * The new "hwspinlock" framework allows the implementation of synchronization primitives on systems where different cores are running different operating systems. See [Documentation/hwspinlock.txt](<https://lwn.net/Articles/435031/>) for more information. 
