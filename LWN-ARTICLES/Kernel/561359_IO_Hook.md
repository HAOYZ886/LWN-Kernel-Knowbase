---
title: I/O Hook
url: https://lwn.net/Articles/561359/
date: "July 30, 2013"
...
While the in-kernel interface may be useful, it will probably be more common to control this facility through the debugfs interface. The module provides a set of files through which overrides can be set up; see [the documentation file](<https://lwn.net/Articles/561363/>) for details on the syntax. The debugfs interface also provides a mechanism by which a simulated interrupt can be delivered to the driver; if an interrupt number is given to the system (by writing it to the appropriate debugfs file), that interrupt will be triggered once the overrides are enabled. 
...
Similarly, overrides can be used to test error paths by injecting various types of errors into the system. Error paths are often difficult to exercise; there are almost certainly large numbers of error paths in the kernel that have never been executed. Code that has never run has a higher-than-average chance of containing bugs. The [fault injection framework](<https://lwn.net/Articles/209257/>) can be used to test a wide range of error paths, but it is not comprehensive; the I/O hook module could be useful to fill in the gaps. 
