---
title: The 4.1 merge window closes
url: https://lwn.net/Articles/642039/
date: "April 28, 2015"
category: "Releases-4.1"
author: "By Jonathan Corbet April 28, 2015"
---
...
By the time Linus released [4.1-rc1](<https://lwn.net/Articles/641911/>) and closed the merge window for this development cycle, some 10,659 non-merge changesets had been pulled into the mainline repository. That makes 4.1 a reasonably busy development cycle, but far from the busiest; 3.19 had 11,400 changes during the merge window, and 3.15, the record holder, had just over 12,000. Even if 4.1 is not a record breaker, though, anybody who worried that 4.0 signaled a general slowdown in kernel development can rest easier now. 

Only about 900 of those changesets were pulled since [last week's summary](<https://lwn.net/Articles/641016/>), but there were some interesting changes buried in that last batch. Some of the more significant, user-visible changes include: 

  * The XFS filesystem has gained `RENAME_WHITEOUT` support, meaning that it should now work with the overlayfs union filesystem. Also new in XFS is support for the [`FALLOC_FL_INSERT_RANGE`](<https://lwn.net/Articles/629965/>) option to `fallocate()`, allowing applications to insert a hole into a file. 
  ...
  * The [virtio](<https://lwn.net/Articles/239238/>) subsystem has a new virtio-input driver; its job is to collect and forward input-device events to a virtual device. 

  * The arm64 architecture has gained support for the Advanced Configuration and Power Interface, otherwise known as ACPI. Supporting ACPI for ARM [has been controversial](<https://lwn.net/Articles/574439/>) in the past; many developers would rather see the device tree mechanism used universally for hardware discovery on that architecture. The addition of ACPI happened quietly in the end, though, and it seems likely that there will be servers using ACPI shipping in the near future. That said, there is still some work to do; the [merge commit](<http://git.kernel.org/linus/836ee4874e201a5907f9658fb2bf3527dd952d30>) notes that ""we don't support any peripherals yet, so it's fairly limited in scope."" See [Documentation/arm64/arm-acpi.txt](<https://lwn.net/Articles/642050/>) for a lot of information about ACPI on ARM. 
